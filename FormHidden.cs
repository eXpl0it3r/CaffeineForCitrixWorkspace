...
public partial class FormHidden : Form
{
    ...
    private void Form1_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            var icaClient = new ICAClientClass();
        ...
        {
            MessageBox.Show("An error occurred while registering the ICA CCM Object", "Error on Startup",
        ...
        {
            MessageBox.Show(
                @$"The CCM registry DWORDS (AllowSimulationAPI & AllowLiveMonitoring) are missing from HKLM:\{CcmRegistryPath}",
                "Missing CCM values detected on Startup", MessageBoxButtons.OK, MessageBoxIcon.Error);
            Close();
        }
    ...
    private void tmrTick_Tick(object sender, EventArgs e)
    {
        try
        {
            ...
            for (var itemCount = 0; itemCount < sessionCount; itemCount++)
                try
                {
                    Debug.WriteLine($"Sending keepalive to session: {itemCount}");
                    var session = icaClient.GetEnumNameByIndex(enumHandle, itemCount);
                    var icaSession = new ICAClientClass();
